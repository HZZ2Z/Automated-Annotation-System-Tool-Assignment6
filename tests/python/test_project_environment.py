...
def _fake_project(
    ...
    script = root / "project_env.sh"
    shutil.copy2(ROOT / "project_env.sh", script)
...
def test_project_environment_rejects_shell_functions_as_media_tools(
    ...
    fake_script = fake_root / "project_env.sh"
    fake_python = fake_root / ".venv" / "bin" / "python"
    fake_root.mkdir(parents=True)
    fake_python.parent.mkdir(parents=True)
    shutil.copy2(ROOT / "project_env.sh", fake_script)
    ...
    assert ".tools/ffmpeg/bin" in result.stderr
    assert "PATH" in result.stderr
    assert "README" not in result.stderr


@pytest.mark.parametrize(
    ("python_version", "media_version", "expected_error"),
    (
        ("3.9.18", "6.1.2", "Python >=3.12,<3.15"),
        ("3.11.14", "6.1.2", "Python >=3.12,<3.15"),
