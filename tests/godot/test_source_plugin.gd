...
static func _test_normalized_video_without_model_output(support) -> void:
	...
	support.expect_equal(source.load_texture(0).get_image().get_pixel(0, 0), Color.BLUE, "normal playback retains its existing cached image")
	support.expect(source.has_method("load_image_snapshot_uncached"), "manifest Source supplies an optional uncached image snapshot boundary")
	if source.has_method("load_image_snapshot_uncached"):
		var fresh: Image = source.load_image_snapshot_uncached(0)
		support.expect(fresh != null and fresh.get_pixel(0, 0) == Color.RED, "manifest Source fresh snapshot sees a cached file replacement")
		if fresh != null: fresh.fill(Color.GREEN)
		var next: Image = source.load_image_snapshot_uncached(0)
		support.expect(next != null and next.get_pixel(0, 0) == Color.RED, "fresh snapshot mutation does not change Source pixels")
		support.expect(source.load_image_snapshot_uncached(-1) == null, "fresh snapshot rejects out-of-range playback index")
	...
	if source.has_method("load_image_snapshot_uncached"):
		support.expect(source.load_image_snapshot_uncached(0) == null and "missing" in source.last_error, "fresh snapshot rejects removed files even when playback remains cached")
