...
func run(support, tree: SceneTree) -> void:
	...
	await _test_main_owns_sam_batch_context(support, tree)
...
func _test_main_owns_sam_batch_context(support, tree: SceneTree) -> void:
	var main: Variant = await _mounted_main(tree)
	var root := _make_catalog_source(support, "sam-context-owner")
	support.expect_equal(await main.open_source(root), PackedStringArray(), "SAM context fixture opens")
	main.call("_set_selected_region", "duct-0")
	support.expect(main.has_method("_batch_sam_context"), "Main exposes the read-only SAM Batch context reader")
	if main.has_method("_batch_sam_context"):
		support.expect_equal(main.call("_batch_sam_context"), {
			"key_index": 0, "region_id": "duct-0", "edit_pending": false,
		}, "Main owns current playback, selection and pending-edit identity")
...
func _test_pointer_reads_only_at_commit_boundaries(support, tree: SceneTree) -> void:
	...
	support.expect_equal(store.revision_reads, 4,
		"press and release each compare a cheap before/after revision pair")
	support.expect_equal(viewport.get("_record"), visible_record,
		"an unchanged commit boundary should not refresh viewport annotations")

	edit.active_tool = &"fill"
	main.call("_on_image_pointer_event", press, Vector2(5, 5))
	support.expect_equal(store.revision_reads, 6,
		"Fill press checks its own commit revision")
	support.expect_equal(viewport.get("_record"), visible_record,
		"an unchanged immediate Fill should not refresh committed viewport annotations")
	main.call("_on_image_pointer_event", release, Vector2(5, 5))
	support.expect_equal(store.revision_reads, 8,
		"Fill release also checks revision without a tool-specific branch")
	support.expect_equal(store.reads, 0, "unchanged pointer events never copy entire records")
	await _free_main(main, tree)
...
func _test_catalog_sidebar_and_failed_open_are_transactional(support, tree: SceneTree) -> void:
	...
	support.expect_equal(await main.open_source(root), PackedStringArray(),
	...
	var errors: PackedStringArray = await main.open_source("/tmp/annotool-task9-source-does-not-exist")
...
func _test_sidebar_intents_and_reclassification(support, tree: SceneTree) -> void:
	...
	support.expect_equal(await main.open_source(root), PackedStringArray(), "sidebar intent source should open")
	var sidebar = main.get_node(
		"MainVBox/WorkspaceSplit/ContentSplit/RightSidebarContainer/RightSidebar/AnnotationSidebar"
	)
	var viewport = main.get_node("MainVBox/WorkspaceSplit/ContentSplit/ViewportPanel/AnnotationViewport")
...
func _test_modal_guards_all_main_entrypoints(support, tree: SceneTree) -> void:
	...
	support.expect_equal(await main.open_source(root), PackedStringArray(), "modal guard source should open")
	main.call("_set_selected_region", "duct-0")
	...
	main.call("_run_history_undo")
	main.call("_run_history_redo")
	var export_result: Dictionary = await main.export_package(
		"/tmp/annotool-task9-modal-export-%d" % OS.get_process_id())
	var open_errors: PackedStringArray = await main.open_source(root)
	main.call("_on_file_selected", "/tmp/annotool-task9-modal-video.mp4")
	support.expect(not export_result.get("success",false) and not export_result.get("errors",[]).is_empty() and not open_errors.is_empty(),
...
func _test_keyboard_preview_and_text_focus(support, tree: SceneTree) -> void:
	...
	support.expect_equal(await main.open_source(root), PackedStringArray(), "keyboard fixture should open")
	var viewport = main.get_node("MainVBox/WorkspaceSplit/ContentSplit/ViewportPanel/AnnotationViewport")
	viewport.grab_focus()
	await tree.process_frame
...
func _test_candidate_cleanup_and_transactional_edit(support, tree: SceneTree) -> void:
	...
	support.expect_equal(await main.open_source(root), PackedStringArray(), "transaction baseline should open")
	...
	_replace_plugin(main, "source", "image_sequence_source", SOURCE_PROBE.new())
	var errors: PackedStringArray = await main.open_source(root)
	...
	_replace_plugin(main, "source", "image_sequence_source", SOURCE_PROBE.new())
	errors = await main.open_source(root)
...
func _test_navigation_wrappers_have_one_cancel_owner(support, tree: SceneTree) -> void:
	...
	SOURCE_PROBE.reset("valid_two")
	_replace_plugin(main, "source", "image_sequence_source", SOURCE_PROBE.new())
	support.expect_equal(await main.open_source(root), PackedStringArray(), "single-owner navigation fixture should open")
...
func _test_source_boundary_validation(support, tree: SceneTree) -> void:
	...
	support.expect_equal(await main.open_source(root), PackedStringArray(), "boundary baseline should open")
	...
	SOURCE_PROBE.reset("valid_two")
	_replace_plugin(main, "source", "image_sequence_source", SOURCE_PROBE.new())
	support.expect_equal(await main.open_source(root), PackedStringArray(), "two-frame source should open for later-entry boundary tests")
...
func _test_numeric_sequence_uses_sparse_source_mapping(
	...
	var bridge: Variant = main.get("_edit_context_bridge")
	support.expect_equal(bridge.get_current_frame(), 23,
		"edit context keeps the sparse original frame id")
	support.expect_equal(bridge.get_playback_index(), 1,
		"edit context exposes the contiguous playback index separately")
	var viewport = main.get_node(
		"MainVBox/WorkspaceSplit/ContentSplit/ViewportPanel/AnnotationViewport")
...
func _test_workspace_forwards_configured_source_plugin(
	...
	await main.call("_on_workspace_media_requested", "VID70")
...
func _test_source_dialog_routes_plugin_owned_locators(
	...
	await main.call("_on_file_selected", file_path)
	support.expect_equal(main.get_current_frame(), 0,
	...
	await main.call("_on_directory_selected", directory_path)
	support.expect_equal(main.get_current_frame(), 0,
	...
	support.expect_equal(await main.open_workspace(workspace_root), PackedStringArray(),
		"workspace should catalog a plugin-owned locator")
	await main.call("_on_workspace_media_requested", "custom")
	support.expect_equal(main.get_current_frame(), 0,
