...
func run(support, tree: SceneTree) -> void:
	...
	var errors: PackedStringArray = await main.call("open_source", source_root)
	...
	var export_result: Dictionary = await main.call("export_package", source_root.path_join("review_packages"), "review_export_v1")
	support.expect(export_result.get("success",false), "Main should route export through the configured Feedback plugin: " + str(export_result.get("errors",[])))
	var handoff_path: String = export_result.get("output_path","")
	support.expect(FileAccess.file_exists(handoff_path.path_join("manifest.json")), "Main export should publish the full review package")
	...
	var replacement_errors: PackedStringArray = await main.call("open_source", failed_root)
	...
	await main.call("_flush_workspace_changes")
	await main.call("_on_file_selected", raw_file)
...
func _test_working_mask_navigation_guards(
	...
	var replacement_errors: PackedStringArray = await main.open_source(source_root)
...
func _test_external_source_through_main(support, tree: SceneTree, packed: PackedScene) -> void:
	...
	tree.root.add_child(main)
	await tree.process_frame
	var errors: PackedStringArray = await main.call("open_source", "case.fixture")
