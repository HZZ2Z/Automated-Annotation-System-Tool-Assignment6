...
func run(support, tree: SceneTree) -> void:
	...
	explorer.populate_workspace({
		...
		"media": [
			...
			},
			{
				...
				"source_plugin_id": "numeric_sequence_source",
	...
	support.expect(explorer.has_method("populate_workspace_active_source"),
		"workspace explorer should expose the selected Source frames without replacing video nodes")
	if explorer.has_method("populate_workspace_active_source"):
		support.expect_equal(explorer.populate_workspace_active_source("VID68", {
			"display_name": "VID68 active",
			"source_path": "/tmp/case_root/videos/VID68",
			"frames": [
				{"index": 0, "label": "000016.png", "path": "/tmp/case_root/videos/VID68/000016.png"},
				{"index": 1, "label": "000023.png", "path": "/tmp/case_root/videos/VID68/000023.png"},
			],
			"artifacts": [],
		}), PackedStringArray(),
			"a valid active Source presentation should mount under its workspace video")
		support.expect(_find_item(explorer, "Frames (2)") != null,
			"the selected video should expose its frame summary")
		support.expect(_find_item(explorer, "still.png") != null,
			"mounting current frames must preserve other selectable media")
		support.expect(explorer.select_frame(1),
			"an active workspace frame should remain programmatically selectable")
		support.expect_equal(_selected_frame(explorer), 1,
			"active workspace frame selection should retain playback index metadata")
		var before_invalid_active: Dictionary = explorer.get("_view_model").duplicate(true)
		support.expect(not explorer.populate_workspace_active_source("unknown", {
			"display_name": "bad", "source_path": "/tmp/bad",
			"frames": [{"index": 0, "label": "bad", "path": "/tmp/bad"}],
			"artifacts": [],
		}).is_empty(), "an unknown active media identity should be rejected")
		support.expect_equal(explorer.get("_view_model"), before_invalid_active,
			"failed active Source publication must preserve the workspace tree")
	...
	var valid_workspace: Dictionary = workspace_before.duplicate(true)
	(valid_workspace["media"][0] as Dictionary)["baseline_kind"] = "legacy"
	explorer.populate_workspace(valid_workspace)
	support.expect_equal(explorer.get("_view_model"), workspace_before,
		"invalid optional baseline_kind should preserve the previous tree")
	valid_workspace = workspace_before.duplicate(true)
	(valid_workspace["media"][0] as Dictionary)["source_plugin_id"] = "bad plugin id"
	explorer.populate_workspace(valid_workspace)
	support.expect_equal(explorer.get("_view_model"), workspace_before,
		"invalid optional source_plugin_id should preserve the previous tree")
