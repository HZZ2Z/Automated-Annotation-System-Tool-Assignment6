...
func run(support, tree: SceneTree) -> void:
	...
	var exports = main.get("_review_workflow").exports
	support.expect(exports.controller != null and main.has_method("export_package"),
		"Main and the export dialog should share the asynchronous training controller")
	support.expect(exports._browse.file_mode == FileDialog.FILE_MODE_OPEN_DIR
		and exports._browse.access == FileDialog.ACCESS_FILESYSTEM,
		"training export should choose a parent directory through its owned browser")
