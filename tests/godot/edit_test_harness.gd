...
func mount(support: TestSupport, scene_tree: SceneTree, source_override := "") -> bool:
	...
	for _attempt in range(3):
		...
		await tree.process_frame
	var errors: PackedStringArray = await main.open_source(source_root)
...
func pointer_click_modified(image_position: Vector2, shift_pressed := false, ctrl_pressed := false) -> void:
	_pointer_button(image_position, true, false, shift_pressed, ctrl_pressed)
	_pointer_button(image_position, false, false, shift_pressed, ctrl_pressed)


func pointer_drag_modified(points: Array[Vector2], shift_pressed := false, ctrl_pressed := false) -> void:
	if points.size() < 2:
		return
	_pointer_button(points[0], true, false, shift_pressed, ctrl_pressed)
	for index in range(1, points.size()):
		_pointer_motion(points[index], MOUSE_BUTTON_MASK_LEFT, shift_pressed, ctrl_pressed)
	_pointer_button(points[-1], false, false, shift_pressed, ctrl_pressed)


func click_session_action(action_id: StringName) -> void:
	var actions := tool_panel.get_node_or_null("SessionPanel/Actions") as HBoxContainer
	if actions == null:
		return
...
func install_model_assist_service_factory(factory: Callable) -> PackedStringArray:
	edit_plugin.set("model_assist_service_factory", factory)
	var bridge: Variant = main.get("_edit_context_bridge")
	var context: Dictionary = main.call("_edit_context", store, history, bridge)
	var result: Variant = edit_plugin.activate(context)
	main.call("_on_edit_state_changed", edit_plugin.get_edit_state())
	main.call("_sync_tool_panel")
	return result if result is PackedStringArray else PackedStringArray(["invalid Model Assist reactivation result"])
...
func _pointer_button(
	...
	var local_position: Vector2 = viewport.get_image_transform().image_to_viewport(image_position)
	_parse_mouse_button(
		viewport.get_global_rect().position + local_position,
		pressed,
		double_click,
		MOUSE_BUTTON_LEFT,
		shift_pressed,
...
func _pointer_motion(image_position: Vector2, button_mask: int, shift_pressed := false, ctrl_pressed := false) -> void:
	var local_position: Vector2 = viewport.get_image_transform().image_to_viewport(image_position)
	var event := InputEventMouseMotion.new()
	var global_position := viewport.get_global_rect().position + local_position
	event.position = global_position
