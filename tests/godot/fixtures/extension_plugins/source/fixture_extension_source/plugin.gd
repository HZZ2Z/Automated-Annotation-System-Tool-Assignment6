...
func discover_workspace_media(root: String, _token: Variant) -> Dictionary:
	if FileAccess.file_exists(root.path_join("malformed.discovery")):
		return {"claimed": "not-a-bool", "media": [], "errors": PackedStringArray()}
	if not FileAccess.file_exists(root.path_join("custom.fixture")):
		return {"claimed": false, "media": [], "errors": PackedStringArray()}
	return {
		"claimed": true,
		"media": [{
			...
			"media_id": "custom",
			"media_type": "image",
			"source_path": root.path_join("custom.fixture"),
			"relative_path": "custom.fixture",
		}],
