...
static func run(support) -> void:
	...
	_test_endoscapes_taxonomy_is_stable(support)
...
static func _test_endoscapes_taxonomy_is_stable(support) -> void:
	var file := FileAccess.open("res://core/taxonomy/classes.json", FileAccess.READ)
	var taxonomy: Variant = JSON.parse_string(file.get_as_text()) if file != null else null
	support.expect(taxonomy is Dictionary,
		"the production taxonomy should remain readable JSON")
	if not taxonomy is Dictionary:
		return
	var rows := {}
	for value: Variant in taxonomy.get("classes", []):
		if value is Dictionary:
			rows[String(value.get("id", ""))] = value
	var expected := {
		"grasper": ["instrument", "#ef4444"],
		"scissors": ["instrument", "#f59e0b"],
		"cystic_duct": ["anatomy", "#22c55e"],
		"gallbladder": ["anatomy", "#3b82f6"],
		"unknown": ["region", "#a855f7"],
		"cystic_plate": ["anatomy", "#14b8a6"],
		"calot_triangle": ["anatomy", "#06b6d4"],
		"cystic_artery": ["anatomy", "#e11d48"],
		"tool": ["instrument", "#f97316"],
	}
	for class_id: String in expected:
		var expected_values: Array = expected[class_id]
		var row: Dictionary = rows.get(class_id, {})
		support.expect_equal(
			[row.get("kind"), row.get("color")], expected_values,
			"taxonomy class %s should keep its reviewed kind and color" % class_id)
