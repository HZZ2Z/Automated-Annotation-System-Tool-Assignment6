...
static func run(support) -> void:
	...
	if registry.has_method("list_plugins") and registry.has_method("get_descriptor") and registry.has_method("create_plugin"):
		...
		support.expect_equal(source_descriptors.size(), 4,
			"registry should list all four production source descriptors")
		...
		if descriptor != null:
			...
			support.expect(descriptor.has_capability(&"model_assist"), "descriptor should advertise the single-frame model assist capability")
	...
	support.expect(registry.get_plugin("source", "endoscapes_video_source") != null,
		"Endoscapes video discovery should be installed as a production Source plugin")
