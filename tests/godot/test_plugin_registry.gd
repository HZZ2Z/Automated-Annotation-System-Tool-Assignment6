...
static func run(support) -> void:
	...
	for expected_fragment in ["api_version", "duplicate", "unknown stage", "missing method", "unsafe entry", "invalid JSON", "does not exist", "not a Script", "required field", "cannot be instantiated"]:
