...
func _test_navigation_and_direct_shortcuts() -> void:
	...
	_expect(navigation.plugin.handle_key(_key(KEY_M)), "plain M should activate Model Assist")
	_expect_equal(navigation.plugin.get_active_tool(), &"model_assist", "M must reach the appended Model Assist tool")
	_expect(not navigation.plugin.handle_key(_key(KEY_M, true)), "modified M must not activate the plain Model Assist shortcut")
...
func _fixture() -> Dictionary:
	...
	var context := {
		...
		"get_playback_index": func(): return frame[0],
