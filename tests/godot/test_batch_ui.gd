...
func run() -> void:
	...
	main.review_session_root = "/tmp/part4-batch-ui-%d-%d" % [OS.get_process_id(),Time.get_ticks_usec()]
	root.add_child(main)
	await process_frame
	...
	if main.get("_batch_workflow") != null:
		...
		s.expect_equal(await main.open_source("res://sample/assignment_v1"), PackedStringArray(), "direct source still opens")
		s.expect(workflow.available(), "direct source persists reviews in deterministic user sessions")
...
func _end_to_end(s, main, workflow) -> void:
	...
	s.expect_equal(await main.open_workspace(directory), PackedStringArray(), "persistent workspace opens")
	await main._on_workspace_media_requested("batch_clip")
	...
	# This legacy synthetic fixture asserts exact fixed-copy boundaries.  Poly is
	# the product default and has its own motion/edge UI integration fixture.
	workflow._algorithm.select(2)
	workflow._select_algorithm(2)
	...
	s.expect_equal(plan.get("threshold"), 0.1, "UI default threshold uses the relaxed review profile")
	...
	s.expect_equal(workflow._apply.text, "确认并写入 19 帧", "primary action states that confirm and write are atomic")
	...
	for i in range(40, 60):
		if i != 50:
			s.expect(store.is_verified(i), "batch commit verifies changed target %d" % i)
	s.expect(not store.is_verified(50), "manual keyframe remains independently reviewable")
	...
	s.expect_equal(store.get_corrected_record(59).regions[0]["class"], "batch_demo_wrong", "one undo restores batch annotations")
	s.expect(not store.is_verified(59), "same undo restores batch review state")
	main._run_history_redo()
	s.expect_equal(store.get_corrected_record(59).regions[0], truth.truth_regions["59"], "redo restores batch annotations")
	s.expect(store.is_verified(59), "same redo restores batch review state")
	var path: String = main._workspace_label_store.label_path()
	var saved: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(path))
	s.expect_equal(saved.get("schema_version"), 3, "batch saved with immutable baseline in V3 envelope")
	...
	main._run_history_undo()
	s.expect(not store.is_verified(50), "undo standalone keyframe verification")
	main._run_history_redo()
	s.expect(store.is_verified(50), "redo standalone keyframe verification")
	s.expect_equal(await main._flush_workspace_changes(), PackedStringArray(), "flush after history")
	...
	var export_result: Dictionary = await main.export_package(directory.path_join("handoff"),"training_update_v2")
	s.expect(export_result.get("success",false), "export reviewed batch: " + str(export_result.get("errors",[])))
	if not export_result.get("success",false): return
	var export_path: String = export_result.output_path
	var exported: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(export_path.path_join("manifest.json")))
	s.expect_equal(exported.get("package_type"), "training_update_v2", "reviewed batch uses the verified-only training package")
	...
	blocked.close()
	label_store.set("_path", directory.path_join("blocked/labels.json"))
	await workflow.verify_current()
	...
	label_store.set("_path", original_path)
	await workflow.retry_save()
