...
func run(support) -> void:
	...
	_save_image(root.path_join("000016.png"), Color.GREEN)
	support.expect_equal(source.load_texture(0).get_image().get_pixel(0, 0), Color.RED,
		"normal numeric playback retains cached pixels after replacement")
	support.expect(source.has_method("load_image_snapshot_uncached"),
		"numeric Source supplies an optional uncached image snapshot boundary")
	if source.has_method("load_image_snapshot_uncached"):
		var fresh: Image = source.load_image_snapshot_uncached(0)
		support.expect(fresh != null and fresh.get_pixel(0, 0) == Color.GREEN,
			"numeric fresh snapshot sees replacement pixels through a primed cache")
		if fresh != null: fresh.fill(Color.BLUE)
		var next: Image = source.load_image_snapshot_uncached(0)
		support.expect(next != null and next.get_pixel(0, 0) == Color.GREEN,
			"numeric fresh snapshot is detached from future loads")
		support.expect(source.load_image_snapshot_uncached(2) == null,
			"numeric fresh snapshot checks playback bounds")
