...
func _test_mounted_history_and_repair() -> void:
	...
	main.review_session_root = "/tmp/part4-editing-regression-%d-%d" % [OS.get_process_id(),Time.get_ticks_usec()]
	root.add_child(main)
	await process_frame
	...
	_expect((await main.open_source(source)).is_empty(), "real Main opens fixture source")
	...
	var actions: HBoxContainer = panel.get_node("SessionPanel/Actions")
	...
	_expect((await main._flush_workspace_changes()).is_empty(), "committed content saves before test node disposal")
	main.queue_free()
	await process_frame
