...
func _test_real_pending_dialog_guards_every_navigation_route(
	...
	var cases := [
		...
		{"name": "real Explorer row", "action": func(): await harness.click_explorer_frame(1)},
		{
			...
			"action": func():
				var errors: PackedStringArray = await harness.main.open_source(harness.source_root)
	...
	support.expect_equal(await harness.main.open_source(harness.source_root), PackedStringArray(),
