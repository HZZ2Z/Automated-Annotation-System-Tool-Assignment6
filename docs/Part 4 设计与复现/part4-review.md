...
## Endoscapes 自包含 COCO 包

Endoscapes Source 在 UI 中默认选择 `training_coco_v1`；普通图片/视频 Source 仍保留旧选项。帧范围留空时默认导出全部 Source 帧；仅在明确需要子集时输入原始帧号。程序优先使用当前修正，否则沿用上版基线，再否则保留为无 annotation 的零目标图像。预览会分别显示已审核、未审核/旧版和零目标数量。“确认导出范围”只确认这些语义，不会把帧设为已审核。

```bash
source project_env.sh
"$PROJECT6_PYTHON" python/part4.py export \
  --session "/absolute/path/endoscapes/label/endoscapes_test_video_162.json" \
  --source-root "/absolute/path/endoscapes" \
  --output "/absolute/path/export-parent" \
  --kind training_coco_v1 --task detection
"$PROJECT6_PYTHON" python/part4.py validate-package PACKAGE_DIRECTORY
"$PROJECT6_PYTHON" python/coco_package_smoke.py PACKAGE_DIRECTORY
```

上述命令不写 `--frame` 即导出全部 Source 帧；需要子集时可重复添加 `--frame 6575 --frame 7325`。无标注帧会作为 COCO 零目标负样本，模型组必须根据 `samples[].review_status` 和 `samples[].label_source` 决定是否用于训练。

实例分割需另加 `--task instance_segmentation --segmentation-attested`。当任何目标只有 box、mask 已失效或导入不完整时，预期返回稳定错误并且没有正式目录。包布局、E01–E48 和本轮真实输出见 [training-coco-v1-acceptance.md](training-coco-v1-acceptance.md)。
