...
批量页默认选择 `Poly 光流 + 边缘精修`，算法版本为 `poly-sim-flow-edge-v1`。它对真实 Source 帧做相似度门、DIS 光流和局部边缘精修，生成每帧独立、可编辑的 polygon 候选。它是当前 NumPy/OpenCV 环境可复现的保守基线；候选必须人工检查，真实手术视频的密集精度和人工节时尚未测量。

## 使用

1. 打开 Source，在参考帧绘制或修正至少一个 Poly。
2. 打开“批量”，保留默认“Poly 光流 + 边缘精修”，按需要调整可见的相似帧差异阈值（默认 `0.02`），点击“分析 Poly 光流与边缘”。
3. 检查候选首末帧、逐帧预览、停止原因和边缘接受/光流回退诊断；可以缩短范围，但必须包含参考帧。
...
| 相似度 | 64×64 `INTER_AREA` 灰度 MAD；相邻与固定关键帧都严格 `<` UI 阈值，默认 0.02 |
...
| 光流质量 | 外观、一致性、纹理、锚点 IoU、锚点质量的最小值至少 0.65 |
...
| 批次范围 | 含关键帧最多 30 个连续原始帧号；左右方向分别停止 |

边缘精修的预期性拒绝必须返回原始光流 mask 的精确副本，并记录拒绝原因；UI 显示为“光流回退”。这不会撤销已经通过的光流候选。相反，OpenCV 运行异常、非有限诊断、非法返回结构或声称 fallback 却修改 mask 都会丢弃整份计划。

任一 Poly 的相似度、光流或最终 V1 几何失败，都会停止该方向的整批，不跳过失败帧。已确认帧、原始帧号缺口和尺寸变化也截断相应方向；读图失败、Source 映射/图像/标注/确认状态变化使整份计划失效。算法只使用实际图像快照，没有插帧、生成帧或模型推理。
...
| `polygon_propagation.py` | v2 请求、相似度、双向传播、固定锚点、边缘调用和停止规则；不访问 Store |
...
2026-09-10 的五帧实测只接受 playback 2（原帧 11825），候选范围为 `1..2`；两侧随后由局部光流证据停止。11825 的 GrabCut 因产生孔洞被拒绝，最终精确保留 raw-flow mask。30 帧窗口得到相同的唯一候选，说明本例由质量门提前停止，而不是依赖 30 帧上限。mounted Main 验收覆盖默认选择和阈值、预览、Apply、一次 undo/redo、保存/重开、v2 marker、确认和 auto-next，并检查 1280×800 截图。

只有 keyframe 11800 有实例 mask，目标帧没有独立密集真值。因此 Endoscapes 结果只是实际数据路径和人工可审查性的定性证据，不能声称目标帧 IoU、普遍边缘提升或自动验证。图片、mask、绝对数据集路径和叠加图均不得提交；详细复现边界见 [Endoscapes Poly 验收流程](endoscapes-poly-acceptance.md)。
