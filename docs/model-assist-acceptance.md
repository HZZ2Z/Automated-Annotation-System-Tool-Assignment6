...
这套流程用于验证标注页的单帧 Model Assist 工具。它不为 Batch 提供 SAM；Batch 仍由 `Poly 光流 + 边缘精修` 负责相似帧分析和传播。
...
1. 进入 Edit，选择第八个 `Model Assist` 或按无修饰键 `M`；记录工具显示的 CPU/CUDA badge。
...
最终记录将三类证据分开：自动 fake-worker 回归、真实 SAM smoke、人工可见 UI 闭环。CPU 通过可记为功能闭环 PASS，但 CUDA 性能必须记为 NOT RUN；没有官方 config/checkpoint 或没有 `sam2` 时，真实模型验收仍是 PENDING，不能用 fake worker 代替。
